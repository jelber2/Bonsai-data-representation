# CuPy-accelerated version of bonsai_treeHelpers.py
# Array operations use CuPy (GPU) instead of NumPy (CPU).
# scipy.optimize is kept as-is: scalar optimisers have no direct CuPy equivalent.
# File I/O (memmap / save / load) uses numpy via the _numpy alias.

from bonsai.bonsai_helpers import *

import bonsai.bonsai_globals as bs_glob

# ---------- GPU / CPU array back-end ----------
import numpy as _numpy          # kept for file I/O only
try:
    import cupy as np            # GPU arrays – the main back-end
    import cupy as cp
    try:
        from cupyx.scipy.sparse import csr_array, lil_array
    except ImportError:
        # Older cupyx versions expose _matrix variants only
        from cupyx.scipy.sparse import csr_matrix as csr_array, lil_matrix as lil_array
    CUPY_AVAILABLE = True
except ImportError:
...
# scipy.optimize – kept on CPU (no CuPy equivalent)
from scipy.optimize import brentq, minimize

# ---------- CuPy replacements for scipy.spatial.distance ----------
def _pdist_euclidean(X):
    """Pairwise Euclidean distances, condensed form.  X shape: (n, d)."""
    n = X.shape[0]
    diff = X[:, None, :] - X[None, :, :]  # (n, n, d)
    full = np.sqrt(np.sum(diff ** 2, axis=-1))  # (n, n)
    rows, cols = np.triu_indices(n, k=1)
    return full[rows, cols]


def _squareform(condensed):
    """Convert condensed distance array to full symmetric matrix."""
    n_cond = int(condensed.shape[0])
    # Solve n*(n-1)/2 == n_cond  =>  n = (1 + sqrt(1+8*n_cond)) / 2
    n = int(round((1.0 + (1.0 + 8.0 * n_cond) ** 0.5) / 2.0))
    mat = np.zeros((n, n), dtype=condensed.dtype)
    rows, cols = np.triu_indices(n, k=1)
    mat[rows, cols] = condensed
    mat = mat + mat.T
    return mat


class _DistanceModule:
    """Thin namespace so existing 'distance.pdist' and 'distance.squareform' calls
    keep working without modification."""
    @staticmethod
    def pdist(X, metric='euclidean'):
        return _pdist_euclidean(np.asarray(X))

    @staticmethod
    def squareform(X):
        return _squareform(np.asarray(X))


distance = _DistanceModule()
# ------------------------------------------------------------------
    ...
    def sendMergeChildrenInfo(self, runConfigs, pairInfoTuple, UBInfo, chInfo, xrAsIfRoot_g, WAsIfRoot_g):
        ...
        if runConfigs['mem_friendly']:
            ...
            while (not files_sent) and (tries <= 10):
                _numpy.save(runConfigs['mem_friendly_files'] + '_ltqs.npy', ltqsChildren, allow_pickle=False)
                _numpy.save(runConfigs['mem_friendly_files'] + '_ltqsVars.npy', ltqsVarsChildren,
                        allow_pickle=False)
                try:
                    ltqsChildren = _numpy.load(runConfigs['mem_friendly_files'] + '_ltqs.npy', mmap_mode='r',
                                           allow_pickle=False)
                    ltqsVarsChildren = _numpy.load(runConfigs['mem_friendly_files'] + '_ltqsVars.npy', mmap_mode='r',
                                               allow_pickle=False)
                    files_sent = True
    ...
    def store_tree_info_and_coords(self, tree_folder, coords_folder=None, verbose=False, store_posterior_ltqs=False,
        ...
        if coords_folder is not None:
            ...
            if bs_glob.mem_friendly:
                ...
                ltqs = _numpy.lib.format.open_memmap(ltqs_file, dtype='float64', mode='w+',
                                                 shape=(self.nNodes, bs_glob.nGenes))
                ltqs_vars = _numpy.lib.format.open_memmap(ltqsVars_file, dtype='float64', mode='w+',
                                                      shape=(self.nNodes, bs_glob.nGenes))
            else:
            ...
            if bs_glob.mem_friendly:
                ...
                _numpy.save(ltqs_file, ltqs, allow_pickle=False)
                _numpy.save(ltqsVars_file, ltqs_vars, allow_pickle=False)
    ...
    def getEdgeVertInfo_memfriendly(self, coords_folder=None, verbose=False, store_posterior_ltqs=False,
        ...
        if coords_folder is not None:
            if not store_posterior_ltqs:
                ...
                ltqsVars_file = os.path.join(coords_folder, 'posterior_ltqsVars_vertByGene.npy')
            # Pre-allocate room for the matrices that we are going to write on the disk
            ltqs_mm = _numpy.lib.format.open_memmap(ltqs_file, dtype='float64', mode='w+',
                                                shape=(self.nNodes, bs_glob.nGenes))
            ltqs_vars_mm = _numpy.lib.format.open_memmap(ltqsVars_file, dtype='float64', mode='w+',
                                                     shape=(self.nNodes, bs_glob.nGenes))
        vertInfo = {}
        nodeIndToVertInd = {}
        vertIndCounter = 0
        ...
        if coords_folder is not None:
            ...
            # _numpy.save(ltqsVars_file, ltqsVars, allow_pickle=False)
