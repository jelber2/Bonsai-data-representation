...
def mp_print(*args, **kwargs):
    ...
    if ('WARNING' in kwargs) and kwargs['WARNING']:
        warning = True
    mpi_rank = mp_wrapper.get_process_rank()
...
# Used
def startMP(n_workers=None, verbose=True):
    """Replace for startMPI — initialises the thread pool instead of MPI."""
    mp_wrapper.mp_init(n_workers=n_workers)
    mpiSize = mp_wrapper.get_process_size()
    mpiRank = mp_wrapper.get_process_rank()
    if verbose:
        mp_print("Starting with %d worker thread(s)." % mpiSize, ALL_RANKS=True)
    return mpiRank, mpiSize


def startMPI(verbose=True):
    """Kept for backwards compatibility — delegates to startMP."""
    return startMP(n_workers=None, verbose=verbose)
...
def communicateNodePairInfo(oldPairInfoDict, newPairInfoDict, mpi_rank, allRanksGetInfo=True):
    newPairInfoDicts = mp_wrapper.world_allgather(newPairInfoDict)
...
def communicateGlobalVars():
    ...
    for globalVar in allGlobalVars:
        thisVar = mp_wrapper.bcast(getattr(bs_glob, globalVar), root=0)
...
def getAllDLogLs(dLogLDict, mpi_rank, nChildren):
    ...
    dLogLsDicts = mp_wrapper.world_allgather(dLogLDict)
    if mpi_rank != 0:
        dLogLs = None
    else:
        allDLogLsDict = {}
    ...
    if removeDir and empty:
        try:
            ...
            mp_print("Process %d: Apparently, folder %s was already removed." % (
                mp_wrapper.get_process_rank(), tree_folder), ALL_RANKS=True)
...
def removePickledTrees(pickleFolder, removeDir=False, notRemove=None, base=None):
    ...
    if removeDir:
        try:
            ...
            mp_print("Process %d: Apparently, folder %s was already removed." % (
                mp_wrapper.get_process_rank(), pickleFolder))
...
def broadcastRecursiveStruct(recursiveStruct, root=0):
    try:
        recursiveStruct = mp_wrapper.bcast(recursiveStruct, root=root)
    except RecursionError:
        with RecursionLimit():
            recursiveStruct = mp_wrapper.bcast(recursiveStruct, root=root)
...
def store_scdata_and_communicate_path(scData, specific_results_folder, general_results_folder=None, prefix=None,
    ...
    if mpi_info.rank == 0:
        ...
        scdata_path = mp_wrapper.bcast(scdata_path, root=0)
    else:
        scdata_path = None
