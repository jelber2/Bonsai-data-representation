...
import pandas as pd
import csv
from bonsai_multiproc.bonsai_treeHelpers import Tree, TreeNode, optimiseTStar
...
class SCData:
    ...
    def storeTreeInFolder(self, tree_folder, with_coords=False, verbose=False, all_ranks=False, cleanup_tree=True,
        ...
        mpiRank = mp_wrapper.get_process_rank()
        if cleanup_tree:
    ...
    def communicate_tree_topology(self):
        if mp_wrapper.get_process_rank() == 0:
            # Make copy of tree
            tree_copy = self.tree.copy_tree_topology()
            tree_copy.root.storeParent()
        else:
            tree_copy = None
        tree_copy = mp_wrapper.bcast(tree_copy, root=0)
        if mp_wrapper.get_process_rank() != 0:
            self.tree = tree_copy
...
def do_spr_moveset(scdata_path, args, strategy='determ_random_determ', pickup_intermediate=False):
    ...
    :return:
    """
    mpi_info = mp_wrapper.get_mpi_info()
    # info_dict = None
...
def do_spr_moves_with_postprocessing(scdata_path, args, select_cand, select_target, max_moves=None,
    ...
    if not large_tree:
        ...
        if mpi_info.rank == 0:
            ...
            remain_cands = mp_wrapper.bcast(remain_cands, root=0)
            # intermediate_folder = mp_wrapper.bcast(intermediate_folder, root=0)
        else:
            remain_cands = None
        ...
        if mpi_info.size > 1:
            all_negdlogl_nodeind_tuples = mp_wrapper.gather(negdlogl_nodeind_tuples, root=0)
            if mpi_info.rank != 0:
...
def nnnReorderRandom(args, outputFolder, verbose=False, randomMoves=0,
    ...
    randomMoves = mp_wrapper.bcast(randomMoves, root=0)
    if randomMoves == 0:
        randomTries = 0
    else:
        if (randomTries is None) or ((randomTries > 0) and (randomTries < mp_wrapper.get_n_workers())):
            randomTries = mp_wrapper.get_n_workers()
    np.random.seed(42)
    ...
    if mpiInfo.rank == 0:
        ...
        scData = None
        gc.collect()
    # Each tree exploration is independent: run all in parallel using the thread pool.
    def _run_one_nnn_tree(task):
        _moveCounter = 0
        ...
        while _moveCounter < randomMoves:
            ...
            _nodeIndToNode = {node.nodeInd: node for node in _nodesList}
            _candidatesList = [node for node in _nodesList if (not node.isLeaf) and (not node.isRoot)]
            _nCandidates = len(_candidatesList)
            _randInd = np.random.randint(_nCandidates)
            _nnnLoglik, _optEdgeList, _mostUSInfo = _scData.tree.getOptEdgeList(
                _candidatesList[_randInd], args, finalOptTimes=True, returnEdgelist=True,
                random=True, trackCloseness=False, singleProcess=True, mem_friendly=True)
            if _optEdgeList is None:
                if verbose:
                    _cand = _candidatesList[_randInd]
                    _msg = "node %d" % _cand.nodeInd if _cand.parentNode is None \
                        else "nodes %d and %d" % (_cand.nodeInd, _cand.parentNode.nodeInd)
                    mp_print("Random reconfiguration around %s is skipped because of many connected nodes." % _msg,
                             ALL_RANKS=True)
                continue
            _scData.tree.processOptReconfig(_optEdgeList, _nodeIndToNode, _nodesList, _candidatesList,
                                            _mostUSInfo, trackCloseness=False)
            _currLoglik += _nnnLoglik
            if veryVerbose:
                mp_print("Move %d: Random reconfiguration of the nodes around node %d "
                         "changed the loglikelihood by %f" % (_moveCounter, _randInd, _nnnLoglik))
            if verbose and (_moveCounter % 1000 == 0):
                mp_print("Tree %d, Move %d: Random reconfiguration of next-nearest neighbours. Loglik now: %f" % (
                    task, _moveCounter, _currLoglik), ALL_RANKS=True)
            _moveCounter += 1
        _scData.metadata.loglik = _scData.tree.calcLogLComplete(mem_friendly=True,
                                                                loglikVarCorr=_scData.metadata.loglikVarCorr)
        _loglik = _scData.metadata.loglik
        _scData.storeTreeInFolder(os.path.join(random_folder, 'random_tree_%d' % task), with_coords=False,
                                  verbose=verbose, all_ranks=True)
        _scData = None
        gc.collect()
        if verbose:
            mp_print("Made tree number %d by doing %d random moves with a loglikelihood of %f." % (
                task, _moveCounter, _loglik), ALL_RANKS=True)
        return task, _loglik

    if randomTries > 0:
        # Run all tree explorations in parallel, then select the best
        _all_results = mp_wrapper.map_parallel(_run_one_nnn_tree, list(range(randomTries)))
        treeLogliks = dict(_all_results)
    ...
    stored_tree_ind = mp_wrapper.bcast(stored_tree_ind, root=0)
...
def nnnReorder(args, tmp_folder, stored_tree_ind, maxMoves=100, closenessBound=0.5, verbose=False, mem_friendly=True):
    ...
    while goodMovesLeft and (moveCounter < maxMoves):
        ...
        # Evaluate all candidates in parallel: getOptEdgeList only reads the shared tree,
        # never modifies it, so concurrent access is safe under Python 3.14 free-threading.
        def _eval_one_candidate(ind):
            _dsNode = candidatesList[ind]
            if (_dsNode.cumClosenessNNN is not None) and (_dsNode.cumClosenessNNN <= closenessBound):
                return ind, _dsNode.nodeInd, _dsNode.nnnLoglik, _dsNode.nnnLoglikTopChange, False
            _rdl, _tc = scData.tree.getOptEdgeList(_dsNode, args, finalOptTimes=False,
                                                   returnEdgelist=False, singleProcess=True,
                                                   mem_friendly=mem_friendly)
            if _rdl is None:
                if verbose:
                    _msg = "node %d" % _dsNode.nodeInd if _dsNode.parentNode is None \
                        else "nodes %d and %d" % (_dsNode.nodeInd, _dsNode.parentNode.nodeInd)
                    mp_print("Reconfiguration around %s is skipped because of many connected nodes." % _msg,
                             ALL_RANKS=True)
                _rdl, _tc = -1, False
            return ind, _dsNode.nodeInd, _rdl, _tc, True

        _eval_results = mp_wrapper.map_parallel(_eval_one_candidate, list(range(nCandidates)))

        calcCounter = 0
        maxReorder = (-1, -1e9)
        maxReorderNoTopChange = (-1, -1e9)
        reorderDLogLs = {}
        for _ind, nodeInd, reorderDLogL, topChange, _calcd in _eval_results:
            if _calcd:
                calcCounter += 1
                reorderDLogLs[nodeInd] = (reorderDLogL, topChange)
            if topChange and (reorderDLogL > maxReorder[1]):
                maxReorder = (nodeInd, reorderDLogL)
            elif (not topChange) and (reorderDLogL > maxReorderNoTopChange[1]):
                maxReorderNoTopChange = (nodeInd, reorderDLogL)

        # Cache the newly computed values on each node
        for nodeInd, (rdl, tc) in reorderDLogLs.items():
            nodeOI = nodeIndToNode[nodeInd]
            nodeOI.nnnLoglik = rdl
            nodeOI.nnnLoglikTopChange = tc
...
#         gc.collect()
#     if allRanks:
#         # scData = mp_wrapper.bcast(scData, root=0)
#         scData = broadcastRecursiveStruct(scData, root=0)
#         communicateGlobalVars()
#     return scData
...
def initializeSCData(args, createStarTree=True, allGenes=False, allRanks=True, otherRanksMinimalInfo=False,
    ...
    mpiInfo = mp_wrapper.get_mpi_info(singleProcess=(not allRanks))
    scData = SCData(dataset=args.dataset, filenamesData=args.filenames_data, verbose=args.verbose,
    ...
    if allRanks:
        if not otherRanksMinimalInfo:
            ...
            if mpiInfo.rank != 0:
                ...
                scData.tree = Tree()
            scData.metadata = mp_wrapper.bcast(scData.metadata, root=0)
        communicateGlobalVars()
    return scData
...
#     if (mp_wrapper.get_process_rank() == 0) or singleProcess:
#         if fullPath is None:
...
#             with RecursionLimit():
#                 scData = mp_wrapper.bcast(scData)
    ...
    if mpiRank == 0:
        ...
        metadata = pd.DataFrame(
            {'nGenes': scData.metadata.nGenes, 'nCells': scData.metadata.nCells, 'reconsTime': computationTime,
             'zscoreCutoff': args.zscore_cutoff, 'coresUsed': mp_wrapper.get_process_size(),
...
def loadReconstructedTreeAndData(args, tree_folder, reprocess_data=False, all_genes=False, all_ranks=True,
    ...
    mpi_info = mp_wrapper.get_mpi_info(singleProcess=single_process)
    if reprocess_data:
...
def read_and_filter(data_folder, meansfile, stdsfile, sanityOutput, zscoreCutoff, mpiInfo, tmp_folder=None,
    ...
    if mpiInfo.rank == 0:
        ...
        nGenesOrig = None
    nGenesOrig = mp_wrapper.bcast(nGenesOrig, root=0)  # Communicate number of genes with other processes
    # Get number of cells
    ...
    if mpiInfo.size > 1:
        ...
        if mpiInfo.rank == 0:
            ...
            if len(genes_to_keep) <= 1:
                continueYN = False
                mp_wrapper.bcast(continueYN, root=0)
                if len(genes_to_keep) == 0:
            ...
            continueYN = mp_wrapper.bcast(continueOrBreak, root=0)
            if not continueYN:
                exit()
    ...
    #     ltqsInfo = mp_wrapper.GatherNpUnknownSize(ltqsInfo, root=0)
    #     if mpiInfo.rank == 0:
