...
class Tree:
    ...
    _t_reconfig = {'topo': 0., 'ltqs_none': 0., 'upstream': 0., 'merge_zero': 0., 'renumber': 0.}
    _t_reconfig_n = 0
    _t_getopt_n = 0
    _t_getopt = {'buildNNN': 0., 'calcLogL_orig': 0., 'optTimes1': 0.,
                 'mergeChildrenUB': 0., 'optTimes2': 0., 'calcLogL_new': 0., 'rest': 0.}
    ...
    def processOptReconfig(self, optEdgeList, nodeIndToNode, nodesList, candidatesList, mostUSInfo,
        ...
        Tree._t_reconfig['topo'] += _time.perf_counter() - _t0

        # Recalculate ltqs of all ancestors in the subtree
        _t1 = _time.perf_counter()
        mostUSNode.getLtqsNoneOnly()
        Tree._t_reconfig['ltqs_none'] += _time.perf_counter() - _t1

        # Recalculate ltqs of everything that is upstream of subtree
        _t2 = _time.perf_counter()
        mostUSNode.setLtqsUpstream()
        Tree._t_reconfig['upstream'] += _time.perf_counter() - _t2

        # TODO: Do this only when zerotimechild is added or some edge is removed
        _t3 = _time.perf_counter()
        recursionWrap(self.root.mergeZeroTimeChilds)
        Tree._t_reconfig['merge_zero'] += _time.perf_counter() - _t3

        _t4 = _time.perf_counter()
        recursionWrap(self.root.renumberNodes)
        Tree._t_reconfig['renumber'] += _time.perf_counter() - _t4

        Tree._t_reconfig_n += 1
        if Tree._t_reconfig_n == 20:
            t = Tree._t_reconfig
            n = Tree._t_reconfig_n
            mp_print("[NNN-TIMING] processOptReconfig avg over %d moves: "
                     "topo=%.1fms  getLtqsNoneOnly=%.1fms  setLtqsUpstream=%.1fms  "
                     "mergeZeroTime=%.1fms  renumber=%.1fms" % (
                         n, 1000*t['topo']/n, 1000*t['ltqs_none']/n,
                         1000*t['upstream']/n, 1000*t['merge_zero']/n, 1000*t['renumber']/n))
        self.nNodes = bs_glob.nNodes
    ...
    def getOptEdgeList(self, dsNode, args, finalOptTimes=False, returnEdgelist=False, singleProcess=True, random=False,
        ...
        _tg6 = _tmod.perf_counter()
        newLoglik = tree.calcLogLComplete()
        _tg7 = _tmod.perf_counter()
        dsNode.nnnLoglik = newLoglik - origLoglik

        Tree._t_getopt['buildNNN']        += _tg1 - _tg0
        Tree._t_getopt['calcLogL_orig']   += _tg2 - _tg1
        Tree._t_getopt['optTimes1']       += _tg4 - _tg3
        Tree._t_getopt['mergeChildrenUB'] += _tg5 - _tg4
        Tree._t_getopt['optTimes2']       += _tg6 - _tg5
        Tree._t_getopt['calcLogL_new']    += _tg7 - _tg6
        Tree._t_getopt['rest']            += (_tg3 - _tg2)
        Tree._t_getopt_n += 1
        if Tree._t_getopt_n == 20:
            t = Tree._t_getopt
            n = Tree._t_getopt_n
            mp_print("[NNN-TIMING] getOptEdgeList avg over %d moves: "
                     "buildNNN=%.1fms  calcLogL_orig=%.1fms  optTimes1=%.1fms  "
                     "mergeChildrenUB=%.1fms  optTimes2=%.1fms  calcLogL_new=%.1fms  rest=%.1fms" % (
                         n, 1000*t['buildNNN']/n, 1000*t['calcLogL_orig']/n,
                         1000*t['optTimes1']/n, 1000*t['mergeChildrenUB']/n,
                         1000*t['optTimes2']/n, 1000*t['calcLogL_new']/n,
                         1000*t['rest']/n))
    ...
    def buildTreeNNN(self, dsNode, returnMostUSInfo=False, mem_friendly=True, random=False):
        ...
        _tb_end = _tb_time.perf_counter()
        if not hasattr(Tree, '_t_build_n'):
            Tree._t_build_n = 0
            Tree._t_build = {'aiRoot': 0., 'dsNeighbours': 0., 'usNeighbours': 0., 'treeConstruct': 0.}
        Tree._t_build['aiRoot']        += _tb1 - _tb0
        Tree._t_build['dsNeighbours']  += _tb2 - _tb1
        Tree._t_build['usNeighbours']  += _tb3 - _tb2
        Tree._t_build['treeConstruct'] += _tb_end - _tb3
        Tree._t_build_n += 1
        if Tree._t_build_n == 20:
            t = Tree._t_build
            n = Tree._t_build_n
            mp_print("[NNN-TIMING] buildTreeNNN avg over %d calls: "
                     "aiRoot=%.1fms  dsNeighbours=%.1fms  usNeighbours=%.1fms  treeConstruct=%.1fms" % (
                         n, 1000*t['aiRoot']/n, 1000*t['dsNeighbours']/n,
                         1000*t['usNeighbours']/n, 1000*t['treeConstruct']/n))
...
def findNodeLtqsGivenLeafs(childNodes=None, ltqs_gi=None, ltqsVars_gi=None, t_i=None, return_wbar_gi=False,
    ...
    if childNodes is not None:
        ...
        for cInd, child in enumerate(childNodes):
            _ltqsVars = child.getLtqsVars(mem_friendly=True)
            _ltqs = child.ltqs
            # Guard against CuPy arrays leaking into the numpy code path (e.g. when a
            # tree saved by a CuPy run is loaded and only partially converted).
            if hasattr(_ltqsVars, 'get'):
                _ltqsVars = _ltqsVars.get()
            if hasattr(_ltqs, 'get'):
                _ltqs = _ltqs.get()
            wbar_g = 1 / (_ltqsVars + child.tParent)
            if return_wbar_gi:
                wbar_gi[:, cInd] = wbar_g
            W_g += wbar_g
            xr_g += _ltqs * wbar_g
